...
{ 
  ...
  blinkLed(RED_LED, 1000);  // Allume la LED rouge pendant 1 seconde
  Serial.println("LED RED OFF");

  Serial.println("LED ORANGE ON");
  blinkLed(ORANGE_LED, 500); // Allume la LED orange pendant 0.5 seconde
  Serial.println("LED ORANGE OFF");
  
  Serial.println("LED GREEN ON");  // Allume la LED verte pendant 0.5 seconde
  blinkLed(GREEN_LED, 250); // Allume la LED verte pendant 0.25 seconde
